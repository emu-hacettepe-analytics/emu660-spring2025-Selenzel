...
education_data <- add_covid_period(education_data)
head(education_data)
```
...
population_summary <- population_data %>%
  ...
  summarise(MeanPopulation = mean(Population, na.rm = TRUE), Count = n())
...
education_summary <- education_data %>%
  ...
  summarise(Total = sum(Person), .groups = "drop")

ggplot(education_summary, aes(x = Education_Level, y = Total, color = Education_Level, group = Education_Level)) +
  geom_line(size = 1.2) +
  facet_grid(~Year) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    title = "Education Level Distribution by Year",
    y = "Population",
    x = "Education Level",
